...
{
    class StopMovingBehavior : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            maxItemsGiven = body.inventory.GetItemCount(StopMovingItem.index) * softCapStep;
        ...
        private void LateUpdate()
        {
            ...
            if (hardCapCheck && softCapCheck) maxItemsGiven = Mathf.Min(body.inventory.GetItemCount(StopMovingItem.index) * softCapStep, hardItemsGiven);
            else if (!hardCapCheck && softCapCheck) maxItemsGiven = body.inventory.GetItemCount(StopMovingItem.index) * softCapStep;
        ...
        private void GiveScrap(bool wasElite, bool wasBoss)
        {
            ...
            float[] rolls = new float[body.inventory.GetItemCount(StopMovingItem.index)];
            for (int roll = 0; roll < body.inventory.GetItemCount(StopMovingItem.index); roll++)
            {
                float value;
            ...
            {
                body.master.GetComponent<MinionOwnership>().ownerMaster.GetBodyObject().GetComponent<StopMovingBehavior>().scrapAmount += rolls.Max();
            }
